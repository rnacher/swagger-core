...
public class ReaderTest {
  ...
  public class ResponseExampleResource {
    ...
    @ApiResponses({@ApiResponse(code = 200, message = "test", response = Pet.class,
  ...
  public static void main(String[] args) {
    new ReaderTest().scanOverriddenMethod();
  }
  ...
  private Boolean isValidRestPath(Method method) {
    for (Class<? extends Annotation> item : Arrays.asList(GET.class, PUT.class, POST.class,
        DELETE.class, OPTIONS.class, HEAD.class)) {
  ...
  @Test(
      description = "scan resource (impl) which has the Api annotations only declared in its interface")
  ...
  @Test(
      description = "scan resource (impl) which has the ApiParam annotations only declared in its interface")
  ...
  @Test(
      description = "it should scan parameters with Swagger and JSR-303 bean validation annotations")
  public void scanBeanValidation() {

    Swagger swagger = getSwagger(ResourceWithValidation.class);
    assertNotNull(swagger);

    QueryParameter par = (QueryParameter) swagger.getPaths().get("/303").getOperations().get(0)
        .getParameters().get(0);
    assertTrue(par.getRequired());
    assertEquals(par.getMinimum(), new BigDecimal(10));

    par = (QueryParameter) swagger.getPaths().get("/swagger-and-303").getOperations().get(0)
        .getParameters().get(0);
    assertTrue(par.getRequired());
    assertEquals(par.getMinimum(), new BigDecimal(7));

    par = (QueryParameter) swagger.getPaths().get("/swagger").getOperations().get(0).getParameters()
        .get(0);
    assertTrue(par.getRequired());
    assertEquals(par.getMinimum(), new BigDecimal(7));
  }
  ...
  public void scanBigDecimal() {
    ...
    PathParameter parameter =
        (PathParameter) swagger.getPath("/v1/{param1}").getGet().getParameters().get(0);
  ...
  public void scanClassAndFieldLevelAnnotations() {
    ...
    List<Parameter> subResourceParameters =
        getGet(swagger, "/resource/{id}/subresource1").getParameters();
  ...
  public void scanConsumesProducesValuesWithBothClassLevelAnnotations() {
    ...
    assertEquals(getGet(swagger, "/{id}/value").getConsumes().get(0), APPLICATION_XML);
    assertEquals(getGet(swagger, "/{id}/value").getProduces().get(0), TEXT_PLAIN);
    assertEquals(getGet(swagger, "/{id}/{name}/value").getConsumes().get(0),
  ...
  public void scanConsumesProducesValuesWithRsClassLevelAnnotations() {
    ...
    assertEquals(getPut(swagger, "/{id}/value").getConsumes().get(0), APPLICATION_XML);
    assertEquals(getPut(swagger, "/{id}/value").getProduces().get(0), TEXT_PLAIN);
    assertEquals(getPut(swagger, "/split").getProduces(),
        Arrays.asList("image/jpeg", "image/gif", "image/png"));
    assertEquals(getPut(swagger, "/split").getConsumes(),
        Arrays.asList("image/jpeg", "image/gif", "image/png"));
  }
  ...
  public void scanMultipleConsumesProducesValuesWithApiClassLevelAnnotations() {
    ...
    assertEquals(getGet(swagger, "/{id}").getConsumes(), Arrays.asList(
        MediaType.APPLICATION_XHTML_XML, MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON));
    assertEquals(getGet(swagger, "/{id}").getProduces(), Arrays.asList(
        MediaType.APPLICATION_ATOM_XML, MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML));
  ...
  public void scanMultipleConsumesProducesValuesWithRsClassLevelAnnotations() {
    ...
    assertEquals(getGet(swagger, "/{id}").getConsumes(),
        Arrays.asList(MediaType.APPLICATION_JSON, MediaType.APPLICATION_XML));
    assertEquals(getGet(swagger, "/{id}").getProduces(),
  ...
  public void scanOverriddenMethod() {
    ...
    assertEquals(overriddenMethodWithTypedParam.getParameters().get(0).getDescription(),
        "ID of pet to return child");
  ...
  public void testModelResolverXMLPropertiesName() {
    ...
    myClass.populate("isotonicDrink value", "softDrink value", "isoDrink value",
        "isotonicDrinkOnlyXmlElement value");
  ...
  public void testResponseExample() {
    ...
    assertTrue(swagger.getPath("/v1/example").getGet().getResponses().get("200").getExamples()
        .get("application/json") instanceof JsonNode);
